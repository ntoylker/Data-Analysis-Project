...
[R3,adjR3]=Group56Exe8Fun2(y,yfitPCR,numComponents);
...
% As we can see the linear model cannot adapt well on the data since the
% adjR^2 is equal to -7.557.The best perfomance seems to have stepwise
% model.The preTMS variable seems to have improved the predictions of the
% lasso and stepwise model but not for the linear model.The fitting has been
% improved since the R^2 increased for the lasso and stepwise 
% model but not for the linear model  that uses all the variables.The pcr
% model gives better results than the linear model and by using 99% of the
% total variance performs better than the lasso model but worser than the
% stepwise model.